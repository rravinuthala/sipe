...
static GList *
sipe_media_parse_video_codecs(GSList *sdp_attrs)
{
        int             i = 0;
        const gchar     *attr;
        GList           *codecs = NULL;
	...
	printf("\nPARSING VIDEO CODECS\n");
        while ((attr = sipe_utils_nameval_find_instance(sdp_attrs, "rtpmap", i++))) {
                ...
                SipeMediaType type      = SIPE_MEDIA_VIDEO;
...
static gchar *
sipe_media_sdp_video_codecs_format(GList *codecs)
{
	GString *result = g_string_new(NULL);
...
sipe_media_sdp_candidates_format(GList *candidates, sipe_media_call* call)
{
	...
	if ((call->invite_cnt > 1) && call->remote_candidates) {
		...
		if(a==1)
		{
			first = call->remote_video_candidates->data;
			second = call->remote_video_candidates->next->data;
		}
...
sipe_media_create_sdp(sipe_media_call *call) {
	...
	//Added for video
	GList *video_codecs = sipe_backend_get_video_codecs(call);
	GList *video_candidates = sipe_backend_get_video_candidates(call, call->dialog->with);
	gchar *sdp_video_codecs = sipe_media_sdp_video_codecs_format(video_codecs);
	gchar *sdp_video_codec_ids = sipe_media_sdp_codec_ids_format(video_codecs);
	gchar *sdp_video_candidates = sipe_media_sdp_candidates_format(video_candidates,call);
	guint16 local_video_port = sipe_backend_candidate_get_port(video_candidates->data);
	...
	gchar *body = g_strdup_printf(
		...
		,local_video_port,sdp_video_codec_ids,sdp_video_candidates,sdp_video_codecs);
...
sipe_media_parse_sdp_attributes_and_candidates(sipe_media_call* call, gchar *frame) {
	...
	GSList		*sdp_video_attrs = NULL;
	gchar		*remote_video_ip = NULL;
	guint16		remote_video_port= NULL;
	GList		*remote_video_candidates;
	guint16		val;
	//Completed
	...
	for (ptr = lines; *ptr != NULL; ++ptr) {
		if (g_str_has_prefix(*ptr, "a=")) {
			...
			if(video == 1)
				sdp_video_attrs = sipe_utils_nameval_add(sdp_video_attrs, parts[0], parts[1]);
			else
			...
			if(video == 1)
				remote_video_ip = g_strdup(parts[5]);
			else
				remote_ip = g_strdup(parts[5]);
			g_strfreev(parts);
		} else if (g_str_has_prefix(*ptr, "m=")) {
			gchar **parts = g_strsplit(*ptr + 2, " ", 3);
			val = strcmp("video",parts[0]);
			if(val == 0)
			{
				video = 1;
				remote_video_port = atoi(parts[1]);	
			}
			else
	...
	remote_video_candidates = sipe_media_parse_remote_candidates(sdp_video_attrs);
	...
	if (no_error) {
		...
		call->sdp_video_attrs		= sdp_video_attrs;
		call->remote_video_ip		= remote_video_ip;
		call->remote_video_port		= remote_video_port;
		call->remote_video_candidates	= remote_video_candidates;
	} else {
		sipe_utils_nameval_free(sdp_attrs);
...
sipe_media_parse_remote_codecs(sipe_media_call *call)
{
	printf("\n+++++++++++++++ENTERED into function for parsing++++++++++++++\n");
	GList		*local_codecs = sipe_backend_get_local_codecs(call);
	GList		*remote_codecs;

	//For video
	GList		*local_video_codecs = sipe_backend_get_video_codecs(call);
	GList		*remote_video_codecs;
	...
	printf("\n\n****AUDIO COMPLETED*****\n\n");
	//For Video
	remote_video_codecs = sipe_media_parse_video_codecs(call->sdp_video_attrs);
	remote_video_codecs = sipe_media_prune_remote_codecs(local_video_codecs,remote_video_codecs);
	
	sipe_media_codec_list_free(local_video_codecs);
	sipe_media_codec_list_free(local_codecs);

	if (remote_codecs && remote_video_codecs) {
		sipe_media_codec_list_free(call->remote_codecs);

		call->remote_codecs		= remote_codecs;
		
		//For Video
		sipe_media_codec_list_free(call->remote_video_codecs);
		call->remote_video_codecs = remote_video_codecs;
		//Completed

		if (!sipe_backend_set_remote_codecs(call, call->dialog->with)) {
			printf("ERROR SET AUDIO REMOTE CODECS"); // TODO
			return FALSE;
		}
		
		if (!sipe_backend_set_video_remote_codecs(call, call->dialog->with)) {
			printf("ERROR SET VIDEO REMOTE CODECS"); // TODO
			return FALSE;
		}

		if(sipe_backend_set_send_codec(call))
			printf("\nSET CODEC SUCCESSFULL\n");
		else
			printf("\nERROR SET CODEC\n");


		/*if(sipe_backend_set_video_send_codec(call))
			printf("\nVIDEO CODECS SUCCESSFULLY SET\n");
		else
			printf("\nVIDEO CODECS NOT SET\n");*/

		return TRUE;
	} else {
		sipe_media_codec_list_free(remote_codecs);
		sipe_media_codec_list_free(remote_video_codecs);
...
sipe_media_call_init(struct sipe_core_private *sipe_private, const gchar* participant, gboolean initiator)
{
	...
	call->video_media = sipe_backend_media_new(SIPE_CORE_PUBLIC,
					     /* temporary */ (struct sipe_media_call *)call,
					     participant,
					     initiator);
...
void sipe_media_hangup(struct sipe_core_private *sipe_private)
{
	...
	if (call)
	{
		...
		sipe_backend_media_hangup(call->video_media, FALSE);
	}
}
			      ...
			      const char *participant)
{
	...
	sipe_backend_media_add_stream(call->media, participant,"sipe-voice",
				      SIPE_MEDIA_AUDIO,
				      !call->legacy_mode, TRUE);
	sipe_backend_media_add_stream(call->video_media, participant,"sipe-video",
				      SIPE_MEDIA_VIDEO,
				      !call->legacy_mode, TRUE);
...
sipe_media_incoming_invite(struct sipe_core_private *sipe_private, struct sipmsg *msg)
{
	...
	sipe_backend_media_add_stream(call->media, dialog->with, "sipe-voice",SIPE_MEDIA_AUDIO, !call->legacy_mode, FALSE);
	sipe_backend_media_add_stream(call->video_media, dialog->with, "sipe-video",SIPE_MEDIA_VIDEO, !call->legacy_mode, FALSE);

	sipe_backend_media_add_remote_candidates(call->media, dialog->with, call->remote_candidates);
	sipe_backend_media_add_video_remote_candidates(call->video_media, dialog->with, call->remote_video_candidates);
								   ...
								   struct transaction *trans)
{
	...
	if (msg->response == 183) {
		...
		//FSTL Added for making call
		sipe_backend_media_add_video_remote_candidates(call->video_media, call->dialog->with, call->remote_video_candidates);
