...
typedef struct _sipe_media_call {
	...
	GList				*set_remote_codecs;

	//For VIDEO
	sipe_media			video_media;
	gchar				*remote_video_ip;
	guint16				remote_video_port;

	GSList				*sdp_video_attrs;
...
gboolean sipe_backend_media_add_stream(sipe_media *media, const gchar* participant, const gchar* sess_id,
									   SipeMediaType type, gboolean use_nice,
									   gboolean initiator);

void sipe_backend_media_add_remote_candidates(sipe_media *media, gchar* participant, GList *candidates);

void sipe_backend_media_add_video_remote_candidates(sipe_media *media, gchar* participant, GList *candidates);
...
GList *sipe_backend_get_video_codecs(sipe_media_call* call);
...
GList* sipe_backend_get_video_candidates(sipe_media_call* call, gchar* participant);
